    ...
    try:
        ...
        if (xbmc.getSkinDir() == "skin.confluence"):
            try:
                ...
                xbmc.log(msg=pluginLogHeader + "Unable to set view : Skin dir"+xbmc.getSkinDir(),level=xbmc.LOGDEBUG)
    except:
        Error_message(30031)
        xbmc.log(msg=pluginLogHeader + 'Exception (init): ' + str(sys.exc_info()),level=xbmc.LOGFATAL)
        return
...
def addlinkKmotion(video):
    ...
    videoDate = datetime.datetime.fromtimestamp(time.time() - video['posted']).strftime('%d-%m-%Y')
    xbmc.log(msg=pluginLogHeader + "Video upload date : "+str(videoDate),level=xbmc.LOGDEBUG)
    if "category" in video['tags']:
        genre = video['tags']['category'][0]
        genre = unicode(genre)
    else:
        genre = None
...
if settings.getSetting("username") == "":
    msg = xbmcgui.Dialog()
    msg.ok(unicode(language(30027)), unicode(language(30028)))
    xbmc.executebuiltin('Addon.OpenSettings("plugin.video.kmotion")')
else:
    login()
